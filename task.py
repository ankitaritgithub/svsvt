...
# Asynchronous function to fetch PageSpeed Insights
async def fetch_pagespeed_insights_async(url, session, api_key, strategy, semaphore, retries=3):
    ...
    async with semaphore:
        for attempt in range(retries):
            try:
                await asyncio.sleep(1)  # Delay before the request
                async with session.get(api_url) as response:
                    response.raise_for_status()  # Raise an error for bad responses
                    data = await response.json()
                    return extract_metrics(data, url, url_encoded, strategy)
            except aiohttp.ClientResponseError as e:
                print(f"Error fetching data for {url}: {e.status}. Retrying...")
...
async def main():
    ...
    all_urls = extract_links(start_url)
    ...
    if to_check_404:
        save_to_excel(to_check_404, 'output404resurrections.xlsx')
    ...
    save_to_excel(results, 'output_introspections.xlsx')
