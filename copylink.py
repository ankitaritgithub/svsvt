...
async def main():
    ...
    all_urls = await asyncio.get_event_loop().run_in_executor(None, extract_links, start_url)
    print(f"Extracted {len(all_urls)} URLs from {start_url}.")

    # Step 2: Prepare to check each URL for 404 redirects and fetch PageSpeed Insights concurrently
    ...
    async with aiohttp.ClientSession() as session:
        semaphore = asyncio.Semaphore(10)  # Control concurrency for PageSpeed Insights
        tasks = []

        with ThreadPoolExecutor(max_workers=100) as executor:
            for url in all_urls:
                # Check 404 in a separate thread
                future = executor.submit(check_404, url)
                url_404_result = future.result()
                url, status = url_404_result

                if status == "Redirects to 404":
                    to_check_404.append(url)
                else:
                    # If not a 404, fetch PageSpeed Insights
                    tasks.append(fetch_pagespeed_insights_async(url, session, api_key, "desktop", semaphore))

        # Gather results from PageSpeed Insights tasks
